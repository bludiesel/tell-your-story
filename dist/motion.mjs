...
var HARD_LAYOUTS = /* @__PURE__ */ new Set(["cover", "divider"]);
...
async function main() {
  ...
  const excludes = (src) => /divider/.test(src) && /cover/.test(src);
  ...
  if (!enginePasses) {
    broken.push({
      ...
      label: "the engine",
      problems: [
        !excludes(stepFn) ? "visibleBlocks() does not exclude boards \u2014 every board will eat a press per block" : "the reveal pass does not exclude boards \u2014 boards will animate in"
      ]
    });
  }
  if (asJson) {
  ...
  console.log("");
  console.log(`  ${pages.length} pages \xB7 ${hard.length} rigid (no reveal, no bend) \xB7 ${steps} presses to walk the whole book`);
  if (broken.length) {
    console.log("");
