...
| **[`templates/LAYOUTS.md`](templates/LAYOUTS.md)** | Every one of the 18 layouts, with copyable syntax and the placeholder text to replace. |
...
| `node dist/build.mjs content/every-layout.md output/every-layout.html` | — | **The catalogue.** All 18 layouts rendered in one book. Open it before choosing a layout, and after any change to `layouts.css` — it is the fastest way to see what actually broke. |
